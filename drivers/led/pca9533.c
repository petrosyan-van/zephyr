...
static int pca9533_led_on(const struct device *dev, uint32_t led)
{
	const struct pca9533_config *config = dev->config;
	struct pca9533_data *data = dev->data;
	...
	int ret = ls_update(&config->i2c, led, LS_FUNC_ON);

	if (ret == 0) {
		LOG_DBG("LED%u -> ON (steady)", led);
		engine_release(data, led);
	}
...
static int pca9533_led_off(const struct device *dev, uint32_t led)
{
	const struct pca9533_config *config = dev->config;
	struct pca9533_data *data = dev->data;
	...
	int ret = ls_update(&config->i2c, led, LS_FUNC_OFF);

	if (ret == 0) {
		LOG_DBG("LED%u -> OFF (steady)", led);
		engine_release(data, led);
	}
...
static int pca9533_led_init_chip(const struct device *dev)
{
	...
	data->engine_users[0] = data->engine_users[1] = 0;
